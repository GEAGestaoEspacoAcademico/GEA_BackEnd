...
public class AgendamentoAulaService {
    ...
    public void criar(AgendamentoAulaCreationDTO dto) {
        ...
        Sala sala = salaRepository.findById(dto.salaId()).orElseThrow(
                () -> new RuntimeException("Sala não encontrada com ID: " + dto.salaId()));
        ...
        //Frontend vai validar esse primeiro horario
        ...
        List<JanelasHorario> horariosDisponiveis = janelasHorarioRepository.findDisponiveisPorData(dto.dataInicio())
                    .stream().filter(j -> !j.getHoraInicio().isBefore(janelaHorario.getHoraFim()))
                    .toList();

        AgendamentoAula primeiroAgendamento = new AgendamentoAula();

        primeiroAgendamento.setUsuario(usuario);
        primeiroAgendamento.setSala(sala);
        primeiroAgendamento.setDisciplina(disciplina);
        primeiroAgendamento.setDataInicio(dto.dataInicio());
        primeiroAgendamento.setDataFim(dto.dataFim());
        // primeiroAgendamento.setEvento(dto.isEvento());
        primeiroAgendamento.setJanelasHorario(janelaHorario);

        agendamentoAulaRepository.save(primeiroAgendamento);

        int quantidadeAulasRestantes = dto.quantidade() - 1;

        if (horariosDisponiveis.size() < quantidadeAulasRestantes) {
            throw new RuntimeException("Não há janelas de horário suficientes para o agendamento.");
        }
    ...
    public List<AgendamentoAulaResponseDTO> listarTodos() {
        return agendamentoAulaRepository.findAll().stream().map(this::converterParaResponseDTO)
                .collect(Collectors.toList());
    }

    public AgendamentoAulaResponseDTO buscarPorId(Long id) {
        AgendamentoAula agendamento = agendamentoAulaRepository.findById(id).orElseThrow(
                () -> new RuntimeException("Agendamento de aula não encontrado com ID: " + id));
    ...
    public AgendamentoAulaResponseDTO atualizarAgendamentoAula(Long id,
        ...
        if (dto.janelasHorarioId() != null) {
            JanelasHorario janelasHorario =
                    janelasHorarioRepository.findById(dto.janelasHorarioId())
                            .orElseThrow(() -> new RuntimeException("Janela de horários inválida"));
    ...
    private AgendamentoAulaResponseDTO converterParaResponseDTO(AgendamentoAula agendamento) {
        return new AgendamentoAulaResponseDTO(agendamento.getId(),
                ...
                agendamento.getDisciplina() != null ? agendamento.getDisciplina().getSemestre()
                        : null,
                agendamento.getDisciplina() != null
                        && agendamento.getDisciplina().getCurso() != null
                                ? agendamento.getDisciplina().getCurso().getNomeCurso()
                                : null,
                agendamento.getDisciplina() != null
                        && agendamento.getDisciplina().getProfessor() != null
                                ? agendamento.getDisciplina().getProfessor().getNome()
                                : "Não atribuído",
                agendamento.getDataInicio(), agendamento.getDataFim(), agendamento.getDiaDaSemana(),
